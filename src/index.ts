...
import { mainOperators } from "./operators/mains";

export const aggregate = (arr: [], pipeline: [Pipeline]) => {
  ...
  for (let i = 0; i < pipeline.length; i++) {
    ...
    if (!Object.keys(mainOperators).includes(Object.keys(operator)[0])) {
  ...
  return operators.reduce((acc, operator) => {
    ...
    return mainOperators[key](acc, value);
  }, arr);
};
